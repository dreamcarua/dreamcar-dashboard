...
(function($) {
    ...
    function initButtons() {
        ...
        $('#customerTypeFilter, #funnelTypeFilter, #tariffFilter, #payProviderFilter, #trafficTypeFilter').on('change', function() {
        ...
        // Заполнить опции периодов проекта для выбранного проекта (вызывается и при инициализации,
        // и при смене проекта - чтобы пресеты "Весь проект / Первые 3 дня / Первая/Последняя неделя"
        // были доступны сразу, без переключения проекта)
        function refreshProjectDateOptions(keepSelection) {
            ...
            if (project && project !== 'all' && dates[projectKey]) {
                ...
                $dateFilter.append('<option class="project-date-option" value="project_first_3_days">Первые 3 дня проекта</option>');
                ...
                // Восстановить ранее выбранную проектную опцию (например при первой загрузке из URL)
                if (keepSelection && prevVal && prevVal.indexOf('project_') === 0) {
                    $dateFilter.val(prevVal);
                }
            }
        }

        // Сделать доступным для date-filter.js (при восстановлении из URL)
        window.UTMDashboard.refreshProjectDateOptions = refreshProjectDateOptions;
        ...
        $('#modelFilter').on('change', function() {
            ...
            // Был ли активен проектный пресет ДО перестроения опций
            // (важно: refreshProjectDateOptions удалит .project-date-option,
            //  и если выбранной была project-опция другого проекта - select сбросится
            //  на первую опцию, а dateFilter.range останется со старыми датами -> отсюда нули)
            var wasProjectPreset = (window.UTMDashboard.dateFilter.range === 'custom' &&
                                    ($dateFilter.val() || '').indexOf('project_') === 0);

            // Перестроить опции периодов проекта
            refreshProjectDateOptions(false);
            ...
            // Текущий проект ИЛИ "Все проекты": если до этого был проектный пресет
            // (или select сейчас стоит на project_*) - сбросить период на "Вчера"
            // и СИНХРОНИЗИРОВАТЬ dateFilter, чтобы не остались старые from/to
            var curVal = $dateFilter.val();
            if (wasProjectPreset || (curVal && curVal.indexOf('project_') === 0)) {
        ...
        // Заполнить опции периодов проекта при первой загрузке (для уже выбранного проекта)
        refreshProjectDateOptions(true);
        ...
        $('#clearUtmFilters').on('click', function() {
            ...
            $('#tariffFilter, #payProviderFilter, #trafficTypeFilter').val('all');
            highlightActiveFilters();
    ...
    function highlightActiveFilters() {
        $('#customerTypeFilter, #funnelTypeFilter, #tariffFilter, #payProviderFilter, #trafficTypeFilter').each(function() {
    ...
    function loadData() {
        ...
        const trafficType = $('#trafficTypeFilter').val() || 'all';
    ...
    function updateStats(stats) {
        ...
        // Unique buyers
        var uniqueBuyers = stats.unique_buyers || 0;
        document.getElementById("totalUniqueBuyers").textContent = uniqueBuyers;
        var repeatPct = stats.repeat_buyers_pct;
        var elRpt = document.getElementById("totalRepeatBuyersPct");
        if (elRpt && repeatPct !== null && repeatPct !== undefined && parseFloat(repeatPct) > 0) {
            elRpt.textContent = repeatPct + "% повторних"; elRpt.style.display = "block";
        } else if (elRpt) { elRpt.style.display = "none"; }
