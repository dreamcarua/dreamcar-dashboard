...
function addManualCost() {
    ...
    const formData = {
        ...
        category: $('#costCategory').val() || 'other',  // для finance_transactions
        card_id: parseInt($('#costCardId').val(), 10) || 0, // для автосписания с карты
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success && response.data) {
                ...
                $('#editCostCategory').val(cost.category || 'other');
                $('#editCostCardId').val(cost.card_id || 0);
...
function updateManualCost() {
    const formData = {
        ...
        category: $('#editCostCategory').val() || 'other',
        card_id: parseInt($('#editCostCardId').val(), 10) || 0,
