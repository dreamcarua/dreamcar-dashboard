...
- ⏸ **Фільтр «Платіжка» (`f-pay-provider`) практично мертвий** — `pay_provider` заповнений у 79 із 5 934 вересневих угод (1,3 %). Будь-який вибір ховає ~99 % даних. Рішення Вадима: або заповнювати поле в ETL/вебхуку, або прибрати фільтр з панелі. [15.09.2026]
- ⚪ **Витрати (`dashboard_ads_data`) фільтруються лише по датах, без проєкту** — при виборі розіграшу виручка звужується, а spend ні. Для одночасних циклів ROI/ROAS будуть завищені по витратах. Крок: звузити `adsBaseRange()` по `utm_campaign`/`launch` або додати `project` у `dashboard_ads_data`. [15.09.2026]
- ⚪ **`ads_account_to_executor` не знає про `fortunatos`** — таблиця мапить акаунт → ОДНОГО виконавця, а в CLUB UAH їх двоє. Поки url_tags приходять — не болить; після 15.09 fallback уже не переписує реальні мітки. Крок: або дробити акаунти по виконавцях, або визнати таблицю legacy. [15.09.2026]
