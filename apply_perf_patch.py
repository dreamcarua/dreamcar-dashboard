#!/usr/bin/env python3
"""
PERF patch v1: Refactor UTM/Traffic/Campaigns render functions to use Postgres RPC
instead of fetchAllDealsBatched + JS aggregation. Speedup: 50-100× for aggregated views.

Adds new helpers (aggViaRPC, trafficTypeRPC, kpiSummaryRPC, dailySeriesRPC) and
replaces the bodies of renderTrafficType, renderUtmField, renderCampaigns to use them.
renderOverview/renderAnalytics get KPI-summary RPC for instant top numbers.
...
function _rpcParams() {
  ...
  // Active model fallback (legacy localStorage)
  const m = getActiveModel();
  if (!sp && m && m.match) {
    // can't apply substring on server side here; skip — RPC ignores models
  }
  return {
...
new_utm_field = '''async function renderUtmField(field, title, csvName) {
  ...
  try {
    ...
    // Attach ad spend (only fetch ads, no deals)
    let adMap = new Map();
    try {
