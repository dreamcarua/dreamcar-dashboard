...
try {
    ...
    // Фильтр по типу трафика: paid / organic / all
    $trafficType = $_GET['traffic_type'] ?? 'all';
    if ($trafficType && $trafficType !== 'all') {
        $filters['traffic_type'] = $trafficType;
    }
    ...
    // Учитываем не только пресеты today/yesterday, но и выбор одного дня в календаре (custom)
    $isOneDay = ($dateRange === 'today' || $dateRange === 'yesterday');
    if (!$isOneDay && !empty($dateFrom) && !empty($dateTo)) {
        // Сравниваем календарные даты начала и конца периода (без времени)
        $isOneDay = (substr($dateFrom, 0, 10) === substr($dateTo, 0, 10));
    }
    ...
    // UTM фильтры для графика by_date (исправление: ранее эти фильтры не применялись к графикам по часам/дням)
    foreach (['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content'] as $utmField) {
        if (!empty($filters[$utmField])) {
            $whereConditions[] = "LOWER($utmField) LIKE :tp_filter_$utmField";
            $params["tp_filter_$utmField"] = '%' . strtolower($filters[$utmField]) . '%';
        }
    }
    ...
    $response = [
        ...
        'stats' => [
            ...
            'unique_buyers' => $totalStats['unique_buyers'],
            'repeat_buyers_pct' => $totalStats['repeat_buyers_pct'],
