...
function handleGetManualCost() {
    ...
    try {
        ...
        if ($cost) {
            // Подтянуть category и card_id из связанной finance_transaction
            try {
                $txId = FinanceTransaction::findByAdsDataId($id);
                if ($txId > 0) {
                    $tx = FinanceTransaction::getById($txId);
                    if ($tx) {
                        $cost['category'] = $tx['category'] ?? 'other';
                        $cost['card_id']  = (int) ($tx['card_id'] ?? 0);
                    }
                }
            } catch (Throwable $e) {
                error_log('[handler.get_manual_cost] finance lookup error: ' . $e->getMessage());
            }

            // Дефолты если не нашли связанную транзакцию
            if (!isset($cost['category'])) $cost['category'] = 'other';
            if (!isset($cost['card_id']))  $cost['card_id']  = 0;
...
function handleAddManualCost() {
    ...
    try {
        ...
        // Автосинхронизация в finance_transactions (expense)
        // Ошибки НЕ блокируют основной ответ - ads_data уже сохранён успешно.
        $financeTxId = 0;
        try {
            $financeTxId = FinanceTransaction::createFromManualCost($id, $costData);
        } catch (Throwable $finEx) {
            error_log('[handler.add_manual_cost] Finance sync failed for ads_data_id='
                . $id . ': ' . $finEx->getMessage());
        }
        ...
        sendSuccess(['id' => $id, 'finance_tx_id' => $financeTxId], 'Расход успешно добавлен');
    } catch (Exception $e) {
...
function handleUpdateManualCost() {
    ...
    try {
        ...
        // Синхронизация в finance_transactions
        $financeSynced = false;
        try {
            $financeSynced = FinanceTransaction::updateFromManualCost((int) $costData['id'], $costData);
        } catch (Throwable $finEx) {
            error_log('[handler.update_manual_cost] Finance sync failed for ads_data_id='
                . $costData['id'] . ': ' . $finEx->getMessage());
        }
        ...
        sendSuccess(['finance_synced' => $financeSynced], 'Расход успешно обновлён');
    } catch (Exception $e) {
...
function handleDeleteManualCost() {
    ...
    try {
        ...
        // Soft-delete связанной finance транзакции
        $financeDeleted = false;
        try {
            $financeDeleted = FinanceTransaction::deleteByAdsDataId($id);
        } catch (Throwable $finEx) {
            error_log('[handler.delete_manual_cost] Finance sync failed for ads_data_id='
                . $id . ': ' . $finEx->getMessage());
        }

        $logger->success('Ручной расход удалён', [
            'id' => $id,
            'finance_deleted' => $financeDeleted,
        ]);

        sendSuccess(['finance_deleted' => $financeDeleted], 'Расход успешно удалён');
    } catch (Exception $e) {
