<?php
// === 009_retroactive_auto_expenses.php ===
// finance/migrations/009_retroactive_auto_expenses.php
// НАЗНАЧЕНИЕ: Ретроспективно создать авто-расходы 2% + 10% для всех существующих income транзакций
// СВЯЗИ: finance/core/models/FinanceTransaction.php, миграция 008
// РАЗМЕР: ~80 строк
...
try {
    ...
    if (!$col) {
        $result['error'] = 'Колонка parent_transaction_id не существует. Сначала выполните миграцию 008.';
    ...
    $result['success'] = true;
    $result['message'] = 'Миграция 009 выполнена. Создано: ' . $result['created'] . ', пропущено: ' . $result['skipped'];
