...
ZK.Cards = (function() {
    ...
    function renderCards(cards) {
        ...
        for (var i = 0; i < cards.length; i++) {
            ...
            html += '<div class="fin-card-header">';
            html += '<span class="fin-card-bank">' + escHtml(card.bank_name || '') + '</span>';
            html += '<span class="fin-card-last4">' + last4 + '</span>';
            html += '</div>';
            html += '<div class="fin-card-meta">';
            html += '<span class="fin-card-owner">' + escHtml(card.owner_name || '') + '</span>';
            if (platforms) {
                html += '<span class="fin-card-platforms">' + escHtml(platforms) + '</span>';
            }
            html += '</div>';
            html += '<div class="fin-card-balance-row">';
            html += '<span class="fin-card-balance-val">' + ZK.Format.money(card.balance_uah) + '</span>';
            if (card.limit_uah) {
                html += '<span class="fin-card-limit"> / ' + ZK.Format.money(card.limit_uah) + '</span>';
            }
            html += '</div>';
            html += '<div class="card-balance-bar-wrap">';
            html += '<div class="card-balance-bar" style="width:' + Math.min(100, pct) + '%"></div>';
            html += '</div>';
            html += '<div class="fin-card-actions">';
            if (canWrite) {
