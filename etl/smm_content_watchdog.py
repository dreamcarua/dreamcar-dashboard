...
Джерело правди — LIVE IG Graph API (точність до хвилини), з фолбеком на
dashboard_ig_* якщо API щось не віддав. Дедуп/повтори — dashboard_settings.

Env:
  FB_ACCESS_TOKEN, IG_USER_ID, FB_API_VERSION (default v21.0)
  SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
  SMM_CHAT_ID           — опц., інакше беремо dashboard_settings.smm_watchdog_chat_id
...
def parse_ts(s):
    """IG timestamp -> aware UTC datetime. Приймає ...+0000 та ...Z."""
    if not s:
        return None
    s = s.strip()
    try:
        # '2026-06-30T19:59:16+0000'
        if s.endswith("+0000"):
...
def ig_last_story_ts(ig_id):
    """Макс published_at серед активних сторіз (IG віддає ~24 год активних)."""
...
def ig_last_post_ts(ig_id):
    """Макс published_at серед постів/рілз (media endpoint не повертає сторіз)."""
...
# ===== Supabase (dashboard_settings + fallback IG tables) =====
...
def sb_fallback_last(table, ig_id):
    """Макс published_at із dashboard_ig_* (фолбек якщо IG API дав None)."""
    try:
        r = requests.get(
...
# ===== Pure logic (тестується юніт-тестами) =====
def in_quiet_hours(now_kyiv, start=WORK_START_HOUR, end=WORK_END_HOUR):
    """True якщо ЗАРАЗ тиша (поза робочим вікном [start, end))."""
    return not (start <= now_kyiv.hour < end)


def should_alert(now_utc, last_ts, gap_hours, st, remind_hours=REMIND_HOURS):
    """
    Вирішує, чи слати алерт зараз, і повертає (send: bool, new_state: dict).
    st — попередній стан {"last_alert_at": iso|None}.
    Логіка:
      • gap = now - last_ts. Якщо last_ts None → вважаємо великий розрив (алерт).
      • Якщо gap <= threshold → не в розриві, стан скидається (last_alert_at=None).
      • Якщо в розриві:
          - перший алерт (last_alert_at None) → шлемо;
          - інакше шлемо якщо минуло >= remind_hours з останнього алерту.
    (Тихі години перевіряються ОКРЕМО до виклику — тут не враховуються.)
    """
    prev_alert = None
    ...
    if last_ts is not None:
        ...
        if gap <= gap_hours:
            return False, {"last_alert_at": None, "last_content_ts":
...
def main():
    ...
    ig_id = IG_USER_ID or "17841403783002317"
    ...
    if not chat_id:
        log("❌ Немає SMM_CHAT_ID (ні env, ні dashboard_settings.smm_watchdog_chat_id)"); sys.exit(1)

    # LIVE + fallback
    last_story = ig_last_story_ts(ig_id) or sb_fallback_last("dashboard_ig_stories", ig_id)
    last_post = ig_last_post_ts(ig_id) or sb_fallback_last("dashboard_ig_media", ig_id)
    log(f"  ℹ остання сторіз: {last_story} | останній пост/рілз: {last_post}")
    ...
    for kind, last_ts, thr in (("stories", last_story, STORY_GAP_HOURS),
        ...
        if send:
            msg = build_msg(kind, now_utc, last_ts)
            ok = enqueue_tg(chat_id, msg)
