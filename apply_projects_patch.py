...
# Patch 2: Rewrite loadProjects() + add helpers
old2_marker = "async function loadProjects() {\n  try {\n    const { data } = await sb.from('dashboard_deals').select('project')"
if 'getSelectedProject()' in s and '_fmtProjDate' in s:
    print('  ✓ patch 2 already applied')
elif old2_marker not in s:
...
async function loadTariffs"""
    ...
    if n == 0:
        print('❌ patch 2: regex did not match')
        sys.exit(1)
    print('  ✓ patch 2 applied')

# Patch 3: Update #f-project change handler
...
# Patch 4: Update fetchDealsRange filter logic
...
if 'const sp = getSelectedProject();\n  let fromDate' in s:
    ...
    print('❌ patch 4: anchor not found')
    sys.exit(1)
else:
...
print('Next: git add docs/index.html && git commit -m "feat(dashboard): projects from dashboard_projects table" && git push')
