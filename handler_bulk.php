...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $crmData === null) {
        ...
        // Убрать BOM (Make.com Text aggregator добавляет UTF-8 BOM)
        if (substr($input, 0, 3) === "﻿") { $input = substr($input, 3); }
        $input = trim($input);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $adsData === null) {
        ...
        // FALLBACK: если JSON битый (Make.com не экранирует кавычки внутри строк - пример: ad_name "пʼятий пост "проспав?"")
        // - парсим записи поштучно через split, пропускаем битые
        if ($jsonData === null && !empty($input) && $input[0] === '[') {
            $jsonData = [];
            $inner = substr($input, 1, -1);
            $pieces = explode("},{", $inner);
            ...
            foreach ($pieces as $idx => $piece) {
                if ($totalPieces === 1) {
                    ...
                    $obj = $piece . "}";
                } elseif ($idx === $totalPieces - 1) {
                    $obj = "{" . $piece;
                } else {
                    $obj = "{" . $piece . "}";
                }
                
                $parsed = json_decode($obj, true);
                if ($parsed !== null) {
                    $jsonData[] = $parsed;
                    $okCount++;
                } else {
            ...
            error_log("handler_bulk LENIENT: total=$totalPieces ok=$okCount bad=$badCount");
            if (empty($jsonData)) {
                $jsonData = null;
            }
        }
